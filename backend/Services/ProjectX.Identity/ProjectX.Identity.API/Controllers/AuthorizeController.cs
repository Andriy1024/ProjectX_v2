...
{
    [HttpPost("sign-in"), Consumes("application/x-www-form-urlencoded")]
    public Task<IActionResult> SignIn([FromForm] SignInCommand request)
        => Send(request);

    [HttpPost("sign-up"), Consumes("application/x-www-form-urlencoded")]
    public Task<IActionResult> SignUp([FromForm] SignUpCommand request)
        => Send(request);

    [HttpPost("refresh-token"), Consumes("application/x-www-form-urlencoded")]
